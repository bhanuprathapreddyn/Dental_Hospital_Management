...
select  a.Doctor_Id, d.Doctor_name
from Doctor d
inner join  Appointment a on a.Doctor_Id= d.Doctor_Id;

Select count(patient_id),address from patient
group by address order by count(patient_id);

select count(appointment_id), appointment_date from Appointment
group by appointment_date;

select patient_id from patient union
select patient_id from Appointment;

select p.patient_name, a.appointment_date from patient p, Appointment a
